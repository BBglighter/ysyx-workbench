...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc; s->dnpc = s->pc + imm; IFDEF(CONFIG_FTRACE,ftraceCall(s->dnpc,s->pc)));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (src1 + imm)&~1; IFDEF(CONFIG_FTRACE,if(s->isa.inst == 0x00008067) ftraceRet(s->dnpc,s->pc); else ftraceCall(s->dnpc,s->pc)));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (sword_t)src1 >> ((imm & 0xFFF) - (1 << 10)));
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1 >> (imm & 0xFFF));
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << (imm & 0xFFF));  
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = BITS(((int64_t)(int32_t)src1 * (int64_t)(int32_t)src2),31,0));
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, if(src2 == 0 || (src1 == 0x80000000 && src2 == 0xFFFFFFFF))  
                                                                  R(rd) = -1; else R(rd) = (sword_t)src1 / (sword_t)src2);
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, if(src2 == 0) R(rd) = -1; else R(rd) = src1 / src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, if(src2 == 0) R(rd) = src1; else if(src1 == 0x80000000 && src2 == 0xFFFFFFFF) 
                                                                                              R(rd) = 0; else R(rd) = (sword_t)src1 % (sword_t)src2);
